...
contract("Loan", (accounts) => { 
    ...
    it("should accept lend amount", async () => { 
        ...
        assert(balanceAfter.sub(balanceBefore).toNumber() === amount); // 1000 - 0 === 000 
    }); 
    
    it("should not reimburse if not borrower", async () => { 
        await expectRevert( 
            loan.reimburse({ from: accounts[3], value: amount + interest }), // 5 
    ...
    it("should not reimbures if not exact amount", async () => { 
        await expectRevert( 
            loan.reimburse({ from: borrower, value: 50 }), // 6 
    ...
    it("should not reimburse of loan has not matured", async () => { 
        await expectRevert( 
            loan.reimburse({ from: borrower, value: amount + interest }), // 7 
